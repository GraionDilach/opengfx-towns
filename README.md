...
A NewGRF which OpenGFX default houses with UK Town Set v2.2 to allow combinations with other base sets and offer additional polish prevented due to being tied to TTD's own spritesheet.
...
## Features
 ...
 - Protect multicell buildings inside inner town zones from town autoreplacement to prevent them being heavily underrepresented in an ongoing game. [^1] [^2] [^3]
 - Mix various climate houses together. [^1] [^2]
 - Rework ground sprites so that all ground sprites are composited from an overlay on top of a baseset sprite to prevent drawing artifacts on the edges of cells against extra-zoom basesets. [^2] [^3]
